...
namespace Odd_Quality_of_Life.Patches
{
    internal class PassableObjects
    {
        ...
        public static void ApplyPatches(Harmony harmony)
        {
            ...
            harmony.Patch
            (
                original: AccessTools.Method(typeof(Tree), nameof(Tree.draw), new Type[1] { typeof(SpriteBatch) }),
        ...
        public static void PlayRustleSound(Vector2 tile, GameLocation location, Character character)
        {
            if ((ModEntry.Config.Shake_when_passing) && StardewValley.Utility.isOnScreen(new Point((int)tile.X, (int)tile.Y), 2, location) && character.isMoving())
            {
                    ...
                    if (__instance.IsSprinkler())
                    {
                        ...
                        if (character_bounding_box.Intersects(object_bounding_box))
                        {
                            ...
                            if (Last_shake_data.Character is not null && (!__instance!.modData.TryGetValue(KeyDataShake, out var data) || !float.TryParse((data = "").Split(';')[0], out var maxShake) || maxShake <= 0f))
                            {
                                ...
                                __instance.modData[KeyDataShake] = $"{maxShake};{shakeRotation};{shakeLeft}";
                                PlayRustleSound(__instance.TileLocation, __instance.Location, character);
                            }
                        }
                    }
                }
                ...
                try
                {
                    ...
                    if (farmer is not null)
                    {
                        ...
                        if ((ModEntry.Config.Shake_when_passing == true) && c is not null && ___maxShake == 0f)
                        {
                            ...
                            PlayRustleSound(__instance.Tile, __instance.Location, farmer);
                        }
                    }
                }                
                catch (Exception e)
        ...
        {
            if (((ModEntry.Config != null) && (bush.size.Value == 3) && (bush.inPot.Value != true)))
            {
                return true;
            }
            return false;
        ...
        private static void Prefix_Draw_Bush(Bush __instance)
        {
            if (AnyPassable(__instance))
            {
                isDrawing = true;
        ...
        private static void Postfix_Tree_BoundingBox(Tree __instance, ref Rectangle __result)
        {
            if (isDrawing)
            {
                isDrawing = false;
