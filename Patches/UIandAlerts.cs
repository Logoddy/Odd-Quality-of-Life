            ...
            if (ModEntry.Config.Festival_notifications_heads_up)
            {
                ...
                else
                {
                    if (StardewValley.Utility.isFestivalDay())
                    {
                        ...
                        Game1.showGlobalMessage(ModEntry.TheHelper.Translation.Get("Utility.Festival.HeadsUp", new { festivalName = festivalName, location = festivalLocation, startTime = festivalStartTime, endTime = festivalEndTime }));
                    }
                    else
                    {
                        ...
                        if (data.DisplayName != null && data.StartTime != null && data.StartTime > 0610)
                        {
                            ...
                            Game1.showGlobalMessage(ModEntry.TheHelper.Translation.Get("Utility.Festival.HeadsUpPassive", new { festivalName = festivalName, startTime = festivalStartTime }));
                        }
                    }
                }
            }
        ...
        public static void TimeChanged(object sender, TimeChangedEventArgs e)
        {
            if (ModEntry.Config.Festival_notifications_warning)
            {
                if (StardewValley.Utility.isFestivalDay())
                {
                    ...
                    {
                        Game1.showGlobalMessage(ModEntry.TheHelper.Translation.Get("Utility.Festival.Warning", new { festivalName = festivalName, location = festivalLocation, endTime = festivalEndTime }));
                    }
                }
            }
        }
